		...
		methods: {
			fetchTodos: function fetchTodos() {
				if (this.currentUser) {
					...
					var query = new _leancloudStorage2.default.Query('AllTodo');

					// find 方法是一个异步方法，会返回一个 Promise，之后可以使用 then 方法
					query.find().then(function (todo) {
						var allTodos = todo[0]; // 因为理论上 AllTodos 只有一个，所以我们取结果的第一项
						var id = allTodos.id;
			...
			saveTodos: function saveTodos() {
				...
				var _this = this;
				// 声明一个 Todo 类型
				var AllTodo = _leancloudStorage2.default.Object.extend('AllTodo');
				// 新建一个 Todo 对象
			...
			updateTodos: function updateTodos() {
				...
				var todo = _leancloudStorage2.default.Object.createWithoutData('AllTodo', this.todoList.id);
				// 修改属性
